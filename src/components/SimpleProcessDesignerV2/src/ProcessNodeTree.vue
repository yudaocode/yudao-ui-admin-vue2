<template>
  <div>
    ...
    <ProcessNodeTree
      ...
      :flow-node.sync="currentNode.childNode"
...
const emits = defineEmits(['update:flowNode', 'find:recursiveFindParentNode'])
...
const findFromParentNode = (nodeList, nodeType) => {
  emits('find:recursiveFindParentNode', nodeList, props.parentNode, nodeType)
}

// 递归从父节点中查询匹配的节点
const recursiveFindParentNode = (nodeList, findNode, nodeType) => {
  if (!findNode) {
    return
  }
