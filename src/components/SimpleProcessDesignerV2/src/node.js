import { ref, watch, inject, unref, toRaw } from 'vue'
...
const parseFormCreateFields = (formFields) => {
  const result = []
  if (formFields) {
 ...
 */
export function useFormFieldsPermission(defaultPermission) {
  // 字段权限配置. 需要有 field, title, permission 属性
  ...
  const getNodeConfigFormFields = (nodeFormFields) => {
  ...
  const mergeFieldsPermission = (formFieldsPermisson, formFields) => {
    let mergedFieldsPermission = []
    if (formFields) {
  ...
  const getDefaultFieldsPermission = (formFields) => {
    let defaultFieldsPermission = []
    if (formFields) {
 ...
 */
export function useNodeForm(nodeType) {
  ...
  if (nodeType === NodeType.USER_TASK_NODE || nodeType === NodeType.TRANSACTOR_NODE) {
    configForm.value = {
      ...
      timeDuration: 6, // 默认 6 小时
      maxRemindCount: 1, // 默认提醒 1 次
      buttonsSetting: []
    }
  } else {
    configForm.value = {
   ...
   * 解析候选人参数
   */
  const parseCandidateParam = (candidateStrategy, candidateParam) => {
 ...
 * @description 节点名称配置
 */
export function useNodeName(nodeType) {
  // 节点名称
  const nodeName = ref()
  // 节点名称输入框
  ...
  const blurEvent = () => {
    ...
    nodeName.value = nodeName.value || NODE_DEFAULT_NAME.get(nodeType)
  }
  return {
    nodeName,
    showInput,
...
export function useNodeName2(node, nodeType) {
  // 显示节点名称输入框
  const showInput = ref(false)
  // 节点名称输入框失去焦点
  const blurEvent = () => {
    showInput.value = false
    node.value.name = node.value.name || NODE_DEFAULT_NAME.get(nodeType)
  }
  // 点击节点标题进行输入
  const clickTitle = () => {
    showInput.value = true
 ...
 */
export function useTaskStatusClass(taskStatus) {
  if (!taskStatus) {
    return ''
  }
