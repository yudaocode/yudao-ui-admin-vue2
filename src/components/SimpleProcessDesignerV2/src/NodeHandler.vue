    ...
    <div class="node-handler">
      ...
      >
        ...
        <template slot="reference">
          <div class="add-icon"><svg-icon icon-class="ep:plus" /></div>
        </template>
      </el-popover>
    </div>
  </div>
...
const readonly = inject('readonly') // 是否只读
...
const addNode = (type) => {
  ...
  if (type === NodeType.USER_TASK_NODE || type === NodeType.TRANSACTOR_NODE) {
    ...
    const data = {
      id: id,
      name: NODE_DEFAULT_NAME.get(type),
      showText: '',
      type: type,
  ...
  if (type === NodeType.DELAY_TIMER_NODE) {
    const data = {
      id: 'Activity_' + generateUUID(),
      name: NODE_DEFAULT_NAME.get(NodeType.DELAY_TIMER_NODE),
      showText: '',
  ...
  if (type === NodeType.ROUTER_BRANCH_NODE) {
    const data = {
      id: 'GateWay_' + generateUUID(),
      name: NODE_DEFAULT_NAME.get(NodeType.ROUTER_BRANCH_NODE),
      showText: '',
  ...
  if (type === NodeType.TRIGGER_NODE) {
    const data = {
      id: 'Activity_' + generateUUID(),
      name: NODE_DEFAULT_NAME.get(NodeType.TRIGGER_NODE),
      showText: '',
  ...
  if (type === NodeType.CHILD_PROCESS_NODE) {
    const data = {
      id: 'Activity_' + generateUUID(),
      name: NODE_DEFAULT_NAME.get(NodeType.CHILD_PROCESS_NODE),
      showText: '',
